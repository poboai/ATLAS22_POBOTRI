...
def save_segmentation_nifti_from_softmax(thres_postprocessing: bool, segmentation_softmax: Union[str, np.ndarray], out_fname: str,
    ...
    if region_class_order is None:
        aggregated_results = seg_old_spacing
        seg_old_spacing = aggregated_results.argmax(0)
        ## For Lesion Limit
        lesion_lim = 0
        default_thres = 0.1
        if len(seg_old_spacing[seg_old_spacing>=1]) <= lesion_lim and thres_postprocessing:
            init_size = len(seg_old_spacing[seg_old_spacing>=1])
            seg_old_spacing_final = np.zeros(aggregated_results.shape[1:])
            seg_old_spacing_final[aggregated_results[1] >= default_thres] = 1
            seg_old_spacing = seg_old_spacing_final
            print('Done (thres %f init_size %d targ_size %d) name %s'%(default_thres,init_size,len(seg_old_spacing[seg_old_spacing>=1]),
                                                                                out_fname))
        else:
            print('Done (init_size %d) name %s'%(len(seg_old_spacing[seg_old_spacing>=1]),out_fname))
    ...
    # seg_old_size_postprocessed = ndimage.binary_fill_holes(seg_old_size_postprocessed).astype(np.uint8)
